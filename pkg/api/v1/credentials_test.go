...
var _ = Describe("Credentials", func() {
	Describe("#CreateCredentials", func() {
		...
		When("getting the token returns an error", func() {
			BeforeEach(func() {
				fakeTHDIdentityClient.TokenReturns("", errors.New("error getting token"))
			})
		...
		When("getting the lifecycle returns an error", func() {
			BeforeEach(func() {
				fakeSnowQLClient.GetLCPReturns("", errors.New("error getting LCP"))
			})

			It("returns status bad request", func() {
				Expect(res.StatusCode).To(Equal(http.StatusInternalServerError))
				validateResponse(payloadErrorGettingLCP)
			})
		})
